//alert("loading ... search site . env , page. ace diff ... and cehck");

/*
    reduce time
    remove diff
        for ace diff
    simple switch on website ... no load
    simple switch on app
	...
	function resetUI() {
		...
		messaging.getToken()
			.then(function(currentToken) {
				if (currentToken) {
					console.log('TOKENF : '+currentToken);

					sharedObj.fcm= sharedObj.fcm || {};
					sharedObj.fcm.deviceId = currentToken ;

					/*
                // SUBSCRIBE to a topic
                messaging.subscribeToTopic(currentToken, '/topics/all')
                  .then(function(response) {
                    // See the MessagingTopicManagementResponse reference documentation
                    // for the contents of response.
                    console.log("Successfully subscribed to topic:", response);
                    alert("Successfully subscribed to topic:"+ response);
...
// SAVE AS
// https://github.com/eligrey/FileSaver.js   https://plnkr.co/edit/hJMsVeBXbtIiBEUa8NED?p=preview
...
/// const rootRequest = "https://api.remitgulf.com/";
// ngTouch : https://docs.angularjs.org/api/ngTouch
// ngAnimate : https://docs.angularjs.org/api/ngAnimate

var newApp = angular.module('new1', []);
// ,'infinite-scroll'


/*newApp.config(['$locationProvider', function($locationProvider) {
...
newApp.directive('starRating',function() {
	return {
		restrict : 'A',
...
/*
    <input id="inputImage" type="file" accept="image/*" image="image1"
      resize-max-height="800" resize-max-width="800" resize-quality="0.7" resize-type="image/jpg" ng-image-compress />
    <img ng-src="{{image1.compressed.dataURL}}" />
    // https://github.com/oukan/angular-image-compress
    */
newApp.directive('ngImageCompress', ['$q',
	function($q) {
...
newApp.directive("ngTouchend", function () {
	return {
...
// <button class="btn btn-sm btn-danger" ng-ask-click="Are you sure to delete this record ?" ng-a-click="removeUser($index)">Delete</button>
newApp.directive('ngAskClick', [
	function(){
		return {
			link: function (scope, element, attr) {
				var msg = attr.ngAskClick || "Are you sure?";
				var clickAction = attr.ngAClick;
...
newApp.directive('ngMapAutocomplete', [ '$timeout', function ($timeout) {
	return {
		require: 'ngModel',
		scope: {
			ngModel: '=ngModel',
...
newApp.directive("whenScrolled", function(){
	return{
...
newApp.directive('ngDraggable', function($document,$timeout) {
	return {
		restrict: 'A',
		scope: {
		...
		link: function(scope, elem, attr) {
			...
			$timeout(function () {
				...
				if(!elem[0].hasAttribute("dropzone")){
					elem.css({
						"z-index" : 99999999,
					})
				}
				console.log("checking parent scope");
				...
				var penElem = angular.element('<i class="menu-link fa fa-pencil" style="margin-left: -20px; visibility: visible;position:absolute;  color: black; z-index:99999999; top:0px;left:-8px;"></i>')
				var editPen = penElem;
				var dragData;
				var newelem = angular.element("<div></div>");


				if(scope.dragdata && scope.dragdata !=='{}'){
					// 		console.log("checking dragdata");
					//console.log(scope.dragdata);

					// 		scope.dragdata = scope.dragdata.replace(/\n/g,"");
					// 		scope.dragdata = scope.dragdata.replace(/\t/g,"");
					scope.dragdata = scope.dragdata.replace(/%29/g,"&quot;");
					scope.dragdata = scope.dragdata.replace(/%30/g,"&apos;");

					// 		console.log("before parsed");
					//console.log(scope.dragdata);
					dragData = JSON.parse(scope.dragdata);
					// 		console.log("parsed string");
					//console.log(dragData.code);
					dragData.code = dragData.code.replace(/%28/g,'"');
					dragData.code = dragData.code.replace(/%27/g,"'");
					dragData.code = dragData.code.replace(/%32/g,"{{");
					dragData.code = dragData.code.replace(/%33/g,"}}");
					dragData.code = dragData.code.replace(/%35/g,"\n");
					dragData.code = dragData.code.replace(/%36/g,"\t");
					dragData.code = dragData.code.replace(/%37/g,"\\");

					//dragData.code = html_beautify(dragData.code);

					var dragDataStyle =  JSON.stringify(dragData.style, null,4);
					if(!elem.parentsUntil('.container').is("[ng-draggable]")){
						//	alert("main jo set" +dragData.jo);
						mainParentJo = dragData.jo;

					}
					// 		console.log("main parent jo");
					// 		console.log(dragData);
					/*
						 $(elem).on('click',penElem, function(e) {
						 e.stopPropagation()
						 scope.$apply(function () {
						 console.log(dragData);
						 scope.parentScope.updateReq = dragData;
						 scope.parentScope.updateReq.style = dragDataStyle;
						 scope.parentScope.showComponentEditMenu = true;
						 });
				...
				elem.append(penElem);
				penElem.on("click", function (e) {
					e.stopPropagation()
					scope.$apply(function () {
						var updateReq =dragData;
						updateReq.style = dragDataStyle;
						scope.$emit('updateReq',{
							updateReq : updateReq,
							showComponentEditMenu : true
						})
						/*
							 scope.$parent.$parent.$ctrl.updateReq = dragData;
							 scope.$parent.$parent.$ctrl.updateReq.style = dragDataStyle;
							 scope.$parent.$parent.$ctrl.showComponentEditMenu = true;
							 */
					});
				...
				elem.on('mousedown', function(e) {
					...
					document.querySelector("#coordinates").innerText = " x = "+x+ "y = "+y;
				...
				var dropzoneAttr = 'dropzone';
				var angularUnder = null;
				var nestedDrop = true;
				// Handle drag event
				function mousemove(e) {
					e.stopPropagation();

					if(!elem.parent().is('#mainContainer')){
						// 			console.log("isme aaya.....");
						var offset = elem.offset();
						console.log($('#mainContainer').offset().top+" +   "+$('#mainContainer').offset().left)
						var newTop = offset.top - $('#mainContainer').offset().top;
						var newLeft = offset.left - $('#mainContainer').offset().left;
						console.log("new top and new left "+newTop+" -- "+newLeft);
					...
					console.log(elem[0]);
					console.log("style reset 2 ");
					console.log(elem[0].style.top)
					console.log(elem[0].style.left)
					var bottomRight = document.elementFromPoint(rect.left+rect.width,rect.top+rect.height);
					var topLeft =document.elementFromPoint(rect.left,rect.top);
					var topRight =document.elementFromPoint(rect.left+rect.width,rect.top);
					var bottomLeft=document.elementFromPoint(rect.left,rect.top+rect.height);
					...
					if(!dropped || nestedDrop){
						if(angularUnder){
							angularUnder.css({
								border: ""
							});
							angularUnder = undefined;
						}
						if(topLeft && topLeft == bottomRight && topLeft.hasAttribute(dropzoneAttr)){
							angularUnder =angular.element(bottomRight);
							//angularUnder.append(elem);
							angularUnder.css({
								border :'1px dashed blue'
							});
						}
						if(bottomRight && bottomRight.hasAttribute(dropzoneAttr)){
							// alert("hello i am a dropzone");
							angularUnder =angular.element(bottomRight);
							//angularUnder.append(elem);
							angularUnder.css({
								border :'1px dashed blue'
							});
						}else if(topLeft && topLeft.hasAttribute(dropzoneAttr)){
							angularUnder =angular.element(topLeft);
							angularUnder.css({
								border :'1px dashed blue'
							});

						}
						else if(topRight && topRight.hasAttribute(dropzoneAttr)){
							angularUnder =angular.element(topRight);
							angularUnder.css({
								border :'1px dashed blue'
							});

						}
						else if(bottomLeft && bottomLeft.hasAttribute(dropzoneAttr)){
							angularUnder =angular.element(bottomLeft);
							angularUnder.css({
								border :'1px dashed blue'
							});
							...
							if(angularUnder) {
								///New added
								/*   document.querySelectorAll('.dropzone').forEach(function(dropZ){
									 dropZ.style.border = '1px solid black';
									 });*/
								angularUnder.css({
					...
					console.log("before set position");
					console.log("x  "+x + " y "+y);
					console.log(startX);
					y = e.clientY - startY;
					x = e.clientX - startX;
				...
				function mouseup(e) {
					...
					if(angularUnder){
						...
						console.log(scope.$parent);
						console.log("dom element scope");
						var newStyle = {};
						if(dragDataStyle){
							var oldStyle = JSON.parse(dragDataStyle);
							for(var key in oldStyle){
								newStyle[key] = dragDataStyle[key]
							}
						}
						/*elem.css({
							 //position: 'static',
							 top:(elem.offset().top - angularUnder.offset().top)+"px",
							 left: (elem.offset().left - angularUnder.offset().left)+"px"
							 });
							 */
						...
						elem.css({
							...
							top:'0px',
							left:'0px'
							/*	'margin-top': elem[0].offsetTop,
								 'margin-left':elem[0].offsetLeft
								 */});
						var newTop = elem.offset().top;
						var newLeft = elem.offset().left;

						elem.css({
							top:(oldTop-newTop)+"px",
							left:(oldLeft-newLeft)+"px"
						});
						newStyle.position = 'relative';
						console.log("dragdata style "+dragDataStyle);
						newStyle.top = elem.css('top');
						newStyle.left = elem.css('left');
						...
						var dropData = {
							dropZoneName:angularUnder[0].getAttribute('dropzone'),
							jo:dragData.jo,
							style: JSON.stringify(newStyle).toString()
						};

						var immediateJo = angularUnder.closest("[ng-draggable]").attr('valjo');
						dropData.immediateJo = immediateJo;
						console.log("dropdata check");
						console.log(dropData);
						scope.$emit('changeDropZone',{
							showLoader :true,
							data : dropData
						});
						// scope.$parent.$ctrl.showLoader = true;
						// scope.$parent.$ctrl.changeDropzone(dropData);
				...
				function setPosition() {
					...
					if(angularUnder){
						...
						//container = angularUnder[0].getBoundingClientRect();
						console.log("container angularUnder");
						console.log(angularUnder[0]);
						console.log(container)
					}
					if (container) {
						...
						if(x>container.width - elem[0].offsetWidth){
							x = container.width -elem[0].offsetWidth;
						}
						if(y>container.height - elem[0].offsetHeight){
							y = container.height - elem[0].offsetHeight;
						}
...
newApp.directive('onFinishRender', function ($timeout) {
	return {
		restrict: 'A',
		link: function (scope, element, attr) {
			if (scope.$last === true) {
...
newApp.directive('ngJsonExportExcel', function() {
	return {
		restrict: 'AE',
		scope: {
			data: '=',
		...
		link: function(scope, element) {
			...
			// added : https://stackoverflow.com/questions/8493195/how-can-i-parse-a-csv-string-with-javascript-which-contains-comma-in-data
			...
			function _bodyData() {
				...
				angular.forEach(scope.data, function(dataItem) {
					...
					if (scope.nestedDataProperty && dataItem[scope.nestedDataProperty] && dataItem[scope.nestedDataProperty].length) {
...
var ccConfig={
	cacheMinutes : 30, // 3*60,   // 1 minute
	allowCache : false,
	allowTranslationCache : false,
	loadRootStateFromCache : false,
};
var loginObject={};
var sharedObj={
	pageMenu:{entries:[], langData:{} },

	tkObj:{

	}
}
sharedObj.fcmState = false;
...
// localStorage.getItem('appJs')
let network = true;
// check network : TODO
//debugger;


if( network && localStorage.getItem('myarApp_'+vAppJs.ve) ){

	// alert('cache');
	// TODO calculate cache time

	script = document.createElement( "script" );
	script.text = localStorage.getItem('myarApp_'+vAppJs.ve);
	document.head.appendChild( script );

	// if network .. update app.js
	/*$.get("all/serverApp.js", function(data, status){
    //        alert("Data: " + data + "\nStatus: " + status);
      localStorage.setItem('serverApp',data);
    });*/

/* 
 
 // TO AVOID DUPL;ICATE SW.JS 
 
	//    get new version
	var code = $.ajax({
		type: "GET",
		// TODO :
		url: rUrl+ "/myarApp_"+vAppJs.ve+".js",
		async: false
	}).responseText;
	//    script = document.createElement( "script" );
	//    script.text = code;
	//    document.head.appendChild( script );
	localStorage.setItem('myarApp_'+vAppJs.ve ,code);
*/



}else{
//    alert('file .. mke it sync load ');


	// DOC 0: sync req .. async will loose the context and load rest of the page and throw exception
	var code = $.ajax({
		type: "GET",
//        url: "all/serverApp.js",
		url: rUrl+ "/myarApp_"+vAppJs.ve+".js",
		async: false
	}).responseText;
	script = document.createElement( "script" );
	script.text = code;
	document.head.appendChild( script );
//    localStorage.setItem('serverApp',code);
	localStorage.setItem('myarApp_'+vAppJs.ve ,code);
